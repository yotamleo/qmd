...
import { disposeDefaultLlamaCpp, getDefaultLLM, setDefaultLLM, getDefaultLlamaCpp, setDefaultLlamaCpp, LlamaCpp, withLLMSession, pullModels, DEFAULT_MODEL_CACHE_DIR, resolveEmbedModel, resolveGenerateModel, resolveRerankModel, resolveModels, inspectGgufFile } from "../llm.js";
...
function getStore(): ReturnType<typeof createStore> {
  if (!store) {
    ...
    // Untrusted project-local models must not be loaded — neither custom local
    // URIs nor the fork's remote embed/rerank/expand endpoints; status still
    // displays the YAML values via resolveModelsForCli (#889).
    const trusted = localConfigIsFullyTrusted();
    const modelsForLlm = trusted ? activeModels : resolveModels();
    setDefaultLLM(createConfiguredLLM(trusted ? config?.models : undefined, {
...
async function showStatus(): Promise<void> {
  ...
  const { cacheDir: mcpCacheDir, pidPath: mcpPidPath } = mcpDaemonPaths();
...
Indexed: ${result.indexed} new, ${result.updated} updated, ${result.unchanged} unchanged, ${result.removed} removed`);
    ...
    if (result.orphanedCleaned > 0) {
      logInfo(`Cleaned up ${result.orphanedCleaned} orphaned content hash(es)`);
    }
  }
