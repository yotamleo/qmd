...
export function setConfigIndexName(name: string): void {
  ...
  // Windows absolute paths (C:\..., \\server\share) are already
  // absolute -- skip resolve() for those (POSIX path.resolve doesn't recognize a
  // drive letter as absolute and would wrongly prefix it with cwd) and sanitize
  // `:` and `\` alongside `/` (himmel fork HIMMEL-1323 resync).
